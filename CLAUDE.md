...
基于 Claude Code 插件系统构建的**银行测试自动化工具集**，提供从需求分析到用例生成的全流程测试左移解决方案。核心是 `qe-toolkit` 插件（`plugins/qe-toolkit/`），包含六个核心 Skills 和六个工作流命令。
...
2. **插件级**：`plugins/qe-toolkit/.claude-plugin/plugin.json` - 插件元数据（当前 v1.3.0）

### 六个核心 Skills

所有 Skill 位于 `plugins/qe-toolkit/skills/` 下：
...
完整模式各 Skill 之间通过标准化 YAML 格式通信，定义在 `plugins/qe-toolkit/references/artifact-schemas/`：
...
**总览文档**：`plugins/qe-toolkit/references/artifact-schemas/00-overview.md`
...
4. `commands/<command-name>.md`（如需工作流命令，使用 `/qe-` 前缀）
5. `.claude-plugin/marketplace.json` 和 `plugins/qe-toolkit/.claude-plugin/plugin.json`（版本号 + 描述）
6. `plugins/qe-toolkit/README.md`、`commands/help.md`、`commands/status.md`
...
- **插件级**：`plugins/qe-toolkit/references/` — 公共参考文档（如 `artifact-schemas/`）
- **Skill 级**：`plugins/qe-toolkit/skills/<name>/references/` — Skill 专属参考文档（编号前缀 `01-xxx.md`）
...
- 详细规则：`plugins/qe-toolkit/skills/devplan-analyzer/references/01-interface-validation.md`
...
| artifact-schemas 总览 | `plugins/qe-toolkit/references/artifact-schemas/00-overview.md` |
| req-parser Skill | `plugins/qe-toolkit/skills/req-parser/SKILL.md` |
| api-generator 参考 | `plugins/qe-toolkit/skills/api-generator/references/` |
| devplan-analyzer 参考 | `plugins/qe-toolkit/skills/devplan-analyzer/references/` |
