...
完整服务列表参见 `references/02-service-mapping.md`。
...
1. **Service层信息**
   ...
   - ⚠️ 如Service层缺失: 自动创建或提示用户(参见 `references/05-edge-cases.md`)
...
详细原理参见 `references/03-model-instantiation.md`。
...
参见 `references/05-edge-cases.md` 的自动生成解决方案。

### 4. 测试数据缺失

参见 `references/05-edge-cases.md` 的占位符和数据准备策略。
...
详细实施指导:
- **`references/02-service-mapping.md`** - 完整服务映射表和自动扩展策略
- **`references/01-test-code-template.md`** - 完整测试代码模板,包含所有章节和动态参数填充
- **`references/00-yaml-format.md`** - YAML文件格式详情、多环境示例和最佳实践
- **`references/03-model-instantiation.md`** - Model类实例化规则、命名转换算法和原理
- **`references/05-edge-cases.md`** - 边界情况处理(Service层缺失、测试数据缺失)
- **`references/04-advanced-features.md`** - 批量生成、增量更新、覆盖率分析
