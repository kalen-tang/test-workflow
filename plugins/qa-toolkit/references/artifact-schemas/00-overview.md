...
**Artifact Schemas** 是 qa-toolkit 插件中不同 Skills 之间通信的标准化格式规范。每个 Skill 的输出都是一个标准化的 Artifact（制品），作为下一个 Skill 的输入。
...
graph LR
    ...
    A1[PRD 文档] -->|requirement-normalizer| A2[01-normalized-requirement]
    A3[设计文档] -->|design-normalizer| A4[02-normalized-design]
    A5[历史案例] -->|case-normalizer| A6[03-normalized-cases]

    A2 --> B
    A4 --> B
    B -->|requirement-validator| B2[05-validation-report]

    A2 --> C
    B2 --> C
    A6 --> C
    C -->|manual-case-generator| C2[06-manual-test-cases]

    A2 --> D
    A4 --> D
    C2 --> D
    D -->|api-case-generator| D2[07-api-test-cases]
```
...
```mermaid
graph LR
    A[PRD/KM文档] -->|shift-left-analyzer| B[测试左移分析报告]
    B -->|api-case-generator| C[07-api-test-cases]
```
...
|------|------|--------|---------|-----------|------|
| 01 | [normalized-requirement](./01-normalized-requirement-v2.md) | PRD 文档 | requirement-normalizer | requirement-validator<br/>manual-case-generator<br/>api-case-generator | ✅ v2.0 |
| 02 | [normalized-design](./02-normalized-design.md) | 设计文档 | design-normalizer | requirement-validator<br/>api-case-generator | ✅ v1.0 |
| 03 | normalized-cases | 历史案例 | case-normalizer | manual-case-generator | 📋 计划中 |
| 04 | code-diff-report | Code Diff | code-diff-mcp | requirement-validator | 📋 计划中 |
| 05 | validation-report | 01 + 02 + 04 | requirement-validator | manual-case-generator | 📋 计划中 |
| 06 | manual-test-cases | 01 + 03 + 05 | manual-case-generator | 测试执行、api-case-generator | ✅ v1.0 |
| 07 | api-test-cases | 01 + 02 + 06 | api-case-generator | CI/CD 自动化执行 | ✅ v1.0 |
...
**价值**：
- 为 `manual-case-generator` 提供案例复用参考
- 建立测试案例知识库
- 支持相似功能案例推荐
...
**下游使用**：
- `requirement-validator` - 检查需求实现一致性
...
**下游使用**：
- `manual-case-generator` - 生成针对性测试案例
- 项目管理 - 决策是否进入开发阶段
...
**已实现**：`manual-case-generator` Skill
...
PRD 文档 ────────────┐
                      ...
                      ▼
          ┌───────────────────────┐
          │ 01-normalized-requirement │ ◄───── requirement-normalizer
          └───────────────────────┘
                      │
                      ├───────────────────────┐
                      │                       │
...
- [ZA Bank PRD 模板分析](../../skills/requirement-normalizer/references/01-prd-template-analysis.md)
- [ZA Bank 设计文档模板分析](../../skills/design-normalizer/references/01-design-template-analysis.md)

### 参考实现

- [requirement-normalizer SKILL.md](../../skills/requirement-normalizer/SKILL.md)
- [design-normalizer SKILL.md](../../skills/design-normalizer/SKILL.md)
