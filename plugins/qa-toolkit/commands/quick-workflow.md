...
/qa-quick https://km.yourcompany.com/doc/12345
...
调用 `shift-left-analyzer` 分析 KM 开发方案：
...
**执行命令**: `/shift-left-analyzer {doc_path}`
...
**执行命令**: `/api-case-generator {分析报告路径}`
...
建议:
- 手动指定报告路径: /api-case-generator <报告路径>
- 或重新执行步骤1: /shift-left-analyzer ./docs/plan.md
```
...
```bash
# 开发团队提供 KM 文档后立即生成测试
/qa-quick ./docs/new-feature-plan.md
```
...
```bash
# 接口修改后快速更新测试用例
/qa-quick ./docs/api-update-plan.md
```
...
```bash
# 从线上 KM 系统直接生成测试
/qa-quick https://km.company.com/doc/feature-123
```
...
- 需要全面质量保证（使用 `/qa-full`）
- 需要手工测试用例（使用完整模式）
- 需要需求验证（先运行 `/requirement-validator`）
...
- `/qa-status` - 查看工具状态和最近输出
- `/qa-config` - 配置工作流参数
- `/qa-help` - 查看详细帮助
- `/qa-full` - 完整模式工作流（开发中）
- `/shift-left-analyzer` - 独立执行步骤1
- `/api-case-generator` - 独立执行步骤2

## 📖 详细文档

- [快速模式说明](../README.md#快速模式)
- [shift-left-analyzer 文档](../skills/shift-left-analyzer/SKILL.md)
- [api-case-generator 文档](../skills/api-case-generator/SKILL.md)
