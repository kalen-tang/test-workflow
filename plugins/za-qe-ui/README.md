...
za-qe-ui 是一个完整的 UI 自动化测试解决方案，提供从原生录制代码转换到测试执行修复的全流程能力。通过**三个核心 Skills**、**两个工作流命令**和**一个智能 Agent** 的协同工作，帮助测试团队快速将手工录制的 Playwright 脚本转化为可维护的自动化测试代码。
...
**za-qe-ui 插件无法独立使用**，必须在 UI 自动化测试宿主项目中运行。

### 依赖项目

本插件依赖 **[za-ui](http://gitlab.in.za/qa/za-ui.git)** 提供核心运行时框架，包括 `waitForComponentVisible`、`createHybridAssert` 等工具方法及 Midscene AI 集成能力。插件生成的测试代码基于该项目的目录结构约定（`testcase-native/`、`testcase-optimized/`、`component/`、`data/`），脱离宿主项目将因缺少运行时依赖而无法执行。

### 环境准备

```bash
git clone http://gitlab.in.za/qa/za-ui.git
```

完整的环境配置和依赖安装步骤，请参阅 **za-ui 项目的 README.md**。完成环境准备后，在宿主项目根目录下使用本插件的各项命令。
...
/za-qe-ui:ui-generate testcase-native/login-test.test.ts
```
...
```bash
# 1. 转换原生录制代码
/za-qe-ui:ui-generate testcase-native/your-test.test.ts

# 2. 执行测试并自动修复
/za-qe-ui:ui-generate testcase-optimized/your-module/test.test.ts

# 3. 全流程一键执行（转换+测试+修复）
/za-qe-ui:ui-generate testcase-native/your-test.test.ts --full

# 4. 对已有脚本进行增量更新
/za-qe-ui:ui-update 在登录后新增关闭弹窗步骤
```
...
### `/za-qe-ui:ui-generate` - Playwright 完整工作流 ⭐ 推荐
...
# 测试已优化代码（测试模式）
/za-qe-ui:ui-generate testcase-optimized/bib/withdrawal/test.test.ts

# 完整工作流（转换+测试+修复）
/za-qe-ui:ui-generate testcase-native/test.test.ts --full

# 调试模式
/za-qe-ui:ui-generate testcase-optimized/test.test.ts --debug
```
...
# 指定文件并描述改动
/za-qe-ui:ui-update testcase-optimized/bib/withdrawal/cross-border.test.ts 新增填写备注字段

# 修改测试数据
/za-qe-ui:ui-update 将提款金额从 13000 改为 20000

# 新增断言
/za-qe-ui:ui-update 在提交后，验证页面出现"申請成功"文字
...
> 如需全新生成测试脚本，请使用 `/za-qe-ui:ui-generate`

---

## 🔬 核心 Skills

### 1. qe-ui-generate（代码转换器）
...
| 命令 | 文档 | 说明 |
|------|------|------|
| `/za-qe-ui:ui-generate` ⭐ | [ui-generate.md](commands/ui-generate.md) | 完整工作流：转换+测试+修复 |
| `/za-qe-ui:ui-update` | [ui-update.md](commands/ui-update.md) | 增量更新已有测试脚本 |

### Skills 文档

| Skill | 文档 | 说明 |
|-------|------|------|
| qe-ui-generate | [SKILL.md](./skills/qe-ui-generate/SKILL.md) | 原生代码转换规则和优化策略 |
| qe-ui-execute | [SKILL.md](./skills/qe-ui-execute/SKILL.md) | 测试执行和自动修复规则 |
| qe-ui-update | [SKILL.md](./skills/qe-ui-update/SKILL.md) | 增量更新规则和命名规范 |

### Agent 文档

| Agent | 文档 | 说明 |
|-------|------|------|
|  qe-ui-agent | [qe-ui-agent.md](./agents/qe-ui-agent.md) | 端到端工作流协调 Agent |
