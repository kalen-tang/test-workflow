...
### 步骤 5：输出 Markdown 和 PlantUML 文件
```
...
- 将测试功能点 MindMap 保存为 `.puml` 文件
- 将详细测试案例 MindMap 保存为 `.puml` 文件
- 使用脚本自动转换为 `需求ID_测试功能点.xmind` 和 `需求ID_测试案例.xmind`
...
- **`scripts/plantuml_to_xmind.py`** - PlantUML MindMap 转 XMind 格式工具
  - 用法：`uv run scripts/plantuml_to_xmind.py <plantuml文件> <需求ID>`
  - 示例：`uv run scripts/plantuml_to_xmind.py ./result/test-cases.puml BANK-1234`
  - 输出：生成到 PlantUML 文件同目录，文件名为 `需求ID.xmind`
