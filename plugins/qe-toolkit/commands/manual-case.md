...
```bash
# 多个需求文档整合为一个测试方案
/qe-manual ./docs/req1.md ./docs/req2.md ./docs/req3.docx
```
...
```bash
# 生成可视化测试用例用于评审
/qe-manual ./docs/requirement.docx --output ./review
```
...
- `/qe-quick` - 快速模式工作流（KM 文档 → 接口测试）
- `/qe-full` - 完整模式工作流（需求 → 手工用例 → 自动化用例）
- `/doc-reviewer` - 需求文档验证
...
**输入**：
```bash
/qe-manual ./docs/search-feature.md
```
...
**输入**：
```bash
/qe-manual ./docs/ui-requirement.md ./docs/api-requirement.md ./docs/business-rules.docx
```
