...
def analyze(
    ...
    if not command:
        return Decision("ask", "空命令")

    try:
        nodes = parse(command)
    except ParseError as e:
        return Decision("ask", f"解析错误: {e.message}")
...
def _analyze_node(node, config: Config, cwd: Path, *, remote: bool = False) -> Decision:
    ...
    if kind == "command":
        ...
        decisions.extend(_analyze_redirects(node, config, cwd, remote=remote))
        return _combine(decisions) if decisions else Decision("allow", "条件判断")
            ...
            if inner_decision.action != "allow":
                decisions.append(
                    Decision(
                        ...
                        f"算术命令替换: {inner_decision.reason}",
        ...
        decisions.extend(_analyze_redirects(node, config, cwd, remote=remote))
        return _combine(decisions) if decisions else Decision("allow", "算术表达式")

    elif kind == "comment":
        return Decision("allow", "注释")

    elif kind == "empty":
        return Decision("allow", "空")

    else:
        # Unknown node type - default to ask
        return Decision("ask", f"未知语法结构: {kind}")
        ...
        for part in parts:
            ...
            if part_kind == "procsub":
                ...
                if inner_decision.action != "allow":
                    ...
                    return Decision(
                        inner_decision.action,
                        f"进程替换 {direction}(...): {inner_decision.reason}",
                        children=[inner_decision],
                    )
                decisions.append(inner_decision)
                ...
                if (
                    ...
                    if outer_result.action != "allow":
                        ...
                        return Decision("ask", f"命令替换注入风险: {inner_cmd}")
        ...
        if op in (">", ">>", "&>", "&>>", "2>", "2>>"):
            ...
            if redirect_match:
                if redirect_match.decision == "allow":
                    decisions.append(Decision("allow", f"重定向到 {target}"))
                elif redirect_match.decision == "deny":
                    msg = redirect_match.message or redirect_match.pattern
                    decisions.append(Decision("deny", f"重定向到 {target}: {msg}"))
                else:  # ask
                    msg = redirect_match.message or redirect_match.pattern
                    decisions.append(Decision("ask", f"重定向到 {target}: {msg}"))
            else:
                # No rule matched - default ask for output redirects
                decisions.append(Decision("ask", f"重定向到 {target}"))
