...
def approve(reason: str = "所有命令安全") -> dict:
...
def deny(reason: str = "配置规则拒绝") -> dict:
...
def main():
    ...
    if "--help" in sys.argv or "-h" in sys.argv:
        ...
        print(f"Dippy v{__version__} — AI 编码助手智能审批插件")
        print()
        print("用法: dippy [--claude|--gemini|--cursor]")
        print()
        print("从标准输入读取 JSON 钩子载荷，输出 JSON 决策。")
        print()
        print("模式:")
        print("  --claude   强制使用 Claude Code 模式")
        print("  --gemini   强制使用 Gemini CLI 模式")
        print("  --cursor   强制使用 Cursor 模式")
        print("  (未指定时自动从输入检测)")
        print()
        print("选项:")
        print("  --help, -h       显示此帮助")
    ...
    if sys.stdin.isatty():
        print("dippy: 无输入（期望从标准输入接收 JSON）")
        print("运行 'dippy --help' 查看用法。")
        raise SystemExit(0)
